...
class CategoryShiftingController extends Controller
{
    ...
    /**
     * Show the Category Shifting (CS) application form
     * GET /cs
     */
    public function create(Request $request)
    {
        ...
        try {
            ...
            $draftResponse = $this->authorizedRequest()
                ->get($this->backend . '/api/category-shifting/check-draft', ['uid' => $uid]);
            ...
            $occupationResponse = $this->authorizedRequest()
                ->get($this->backend . '/api/category-shifting/get-current-occupation', ['uid' => $uid]);
            ...
            $appDataResponse = $this->authorizedRequest()
                ->get($this->backend . '/api/category-shifting/get-application-data', ['uid' => $uid]);
            ...
            return view('housingTheme.category-shifting.form', [
                'districts' => $districts,
                'payBands' => $payBands,
            ...
            Log::error('Category Shifting Create Error', [
                'error' => $e->getMessage(),
    ...
    /**
     * Store the Category Shifting application
     * POST /cs
     */
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($formData as $key => $value) {
                if ($key === 'cs_file_licence' && $request->hasFile('cs_file_licence')) {
                    $multipartData[] = [
                        'name' => $key,
                        'contents' => fopen($request->file('cs_file_licence')->getRealPath(), 'r'),
                        'filename' => $request->file('cs_file_licence')->getClientOriginalName(),
                    ];
                } else {
                    $multipartData[] = [
                        'name' => $key,
            ...
            $response = $this->authorizedRequest()
                ->attach('cs_file_licence', file_get_contents($request->file('cs_file_licence')->getRealPath()), $request->file('cs_file_licence')->getClientOriginalName())
                ->post($this->backend . '/api/category-shifting/store', $formData);
            ...
            Log::error('Category Shifting Store Error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    private function getApplicantOfficialInfo($uid)
    {
        try {
            ...
            Log::error('Failed to fetch applicant official info', ['error' => $e->getMessage()]);
        }
        return null;
    }
