...
class NewLicenseController extends Controller
{
    ...
    /**
     * Show new license application form
     * GET /new-license
     */
    public function index(Request $request)
    {
        ...
        try {
            ...
            $draftResponse = $this->authorizedRequest()
                ->get($this->backend . '/api/new-license/check-draft', ['uid' => $uid]);
            ...
            $allotmentResponse = $this->authorizedRequest()
                ->get($this->backend . '/api/new-license/allotment-details', ['uid' => $uid]);
            ...
            return view('housingTheme.license-application.new-license', [
                'appType' => 'NL',
                'districts' => $districts,
                'payBands' => $payBands,
            ...
            Log::error('New License Index Error', [
                'error' => $e->getMessage(),
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('scaned_sign')) {
                $builder = $builder->attach('scaned_sign',
                    file_get_contents($request->file('scaned_sign')->getRealPath()),
                    $request->file('scaned_sign')->getClientOriginalName()
                );
            }

            // Attach all other form fields
            foreach ($formData as $key => $value) {
                if ($key !== 'document' && $key !== 'scaned_sign' && !is_null($value)) {
                    $builder = $builder->attach($key, $value);
                }
            }

            $response = $builder->post($this->backend . '/api/new-license/store');
