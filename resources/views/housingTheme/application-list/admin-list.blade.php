                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="cms-card">
                            ...
                            {{-- Flat Type Filter + Application Table --}}
                            <div class="mt-3 mb-4 d-flex align-items-center gap-2 select-field-style">
                                <form method="GET" class="d-flex align-items-center gap-2">
                                    <label for="flat_type" class="form-label mb-0">Select Flat Type:</label>
                                    @php
                                         $currentFlatType = strtoupper(request('flat_type', $flatTypeFilter ?? 'ALL'));
                                    @endphp
                                    <select name="flat_type" id="flat_type" class="form-select form-select-sm" style="width: 200px;" onchange="this.form.submit()">
                                        <option value="ALL" {{ $currentFlatType === 'ALL' || $currentFlatType === '' ? 'selected' : '' }}>All</option>
                                        <option value="A+" {{ $currentFlatType === 'A+' ? 'selected' : '' }}>A+</option>
                                        <option value="A" {{ $currentFlatType === 'A' ? 'selected' : '' }}>A</option>
                                        <option value="B" {{ $currentFlatType === 'B' ? 'selected' : '' }}>B</option>
                                        <option value="C" {{ $currentFlatType === 'C' ? 'selected' : '' }}>C</option>
                                        <option value="D" {{ $currentFlatType === 'D' ? 'selected' : '' }}>D</option>
                                    </select>
                                </form>
                            </div>
