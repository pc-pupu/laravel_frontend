            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    if (!response.already_applied && response.encrypted_uid) {
                        ...
                        // Use the encrypted UID from backend
                        const encryptedUid = response.encrypted_uid;
                        html += '<a href="{{ url("existing-applicant-vs-cs/create") }}/' + encodeURIComponent(encryptedUid) + '" class="btn btn-primary">Next</a>';
                        html += '</div>';
                    } else {
                        html += '<div class="text-right mt-3">';
