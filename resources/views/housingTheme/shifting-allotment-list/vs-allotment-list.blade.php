...
@section('title', 'List of Vertical Shifting Allottees')
        ...
        <div class="col-md-12">
            <div class="cms-card">
                <div class="cms-header">
                    <h3><i class="fa fa-list me-2"></i> List of Vertical Shifting Allottees</h3>
                </div>
                @include('housingTheme.partials.alerts')
                
                <form method="GET" action="{{ route('shifting-allotment-list.vs') }}" id="vs-allotment-list-form">
                    <div class="row mb-4">
                        <div class="col-md-4">
                            <div class="form-floating">
                                <select name="vs_allotment_process_date" id="vs_allotment_process_date" class="form-select" required>
                                ...
                                <label for="vs_allotment_process_date">VS Allotment Process Date <span class="text-danger">*</span></label>
                            </div>
                        </div>
                        <div class="col-md-4" id="vs_allotment_process_no_replace">
                            <div class="form-floating">
                                <select name="vs_allotment_process_no" id="vs_allotment_process_no" class="form-select" 
                                ...
                                <label for="vs_allotment_process_no">VS Allotment Process No. <span class="text-danger">*</span></label>
                            </div>
                        </div>
                    </div>
                </form>

                <div id="vs_allotment_list_replace">
                    @if(!empty($allottees) && count($allottees) > 0)
                        <div class="mb-3">
                            <a href="{{ route('shifting-allotment-list.vs.pdf', [
                                ...
                                'filename' => 'vs_allottee_list_' . time()
                            ]) }}" 
                            target="_blank" 
                            class="btn bg-primary btn-sm px-5 rounded-pill text-white fw-bolder mb-2">
                                <i class="fa fa-download me-2"></i> Download VS Allottee List PDF
                            </a>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(!empty($allottee['encrypted_online_application_id']))
                                                    <a href="{{ route('view-shifting-allotment-details.vs', ['id' => $allottee['encrypted_online_application_id']]) }}" 
...
<script>
document.getElementById('vs_allotment_process_date').addEventListener('change', function() {
    const selectedDate = this.value;
    const processNoSelect = document.getElementById('vs_allotment_process_no');
    
    if (!selectedDate) {
        processNoSelect.innerHTML = '<option value="">Select VS Allotment Process No.</option>';
        processNoSelect.disabled = true;
        document.getElementById('vs_allotment_list_replace').innerHTML = '';
        return;
    }

    // Fetch process numbers via AJAX
    fetch(`{{ route('shifting-allotment-list.vs.process-numbers') }}?allotment_process_date=${selectedDate}`, {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        if (data.status === 'success') {
            let options = '<option value="">Select VS Allotment Process No.</option>';
...
document.getElementById('vs_allotment_process_no').addEventListener('change', function() {
    const selectedDate = document.getElementById('vs_allotment_process_date').value;
    ...
    fetch(`{{ route('shifting-allotment-list.vs.allottees') }}?allotment_process_date=${selectedDate}&allotment_process_no=${selectedProcessNo}`, {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            if (allottees.length > 0) {
                ...
                allottees.forEach((allottee, index) => {
                    ...
                    const viewLink = allottee.encrypted_online_application_id 
                        ? `<a href="{{ url('view_vs_allotment_details') }}/${allottee.encrypted_online_application_id}" class="btn btn-outline-primary btn-sm px-4 rounded-pill fw-bolder" target="_blank"><i class="fa fa-eye me-2"></i> View Details</a>`
                        : 'N/A';
            ...
            document.getElementById('vs_allotment_list_replace').innerHTML = html;
        }
    })
    .catch(error => {
        console.error('Error:', error);
