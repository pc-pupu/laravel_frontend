...
@section('title', 'RHE Category Shifting Allotment')
        ...
        <div class="col-md-12">
            <div class="cms-card">
                <div class="cms-header">
                    <h3><i class="fa fa-key me-2"></i> RHE Category Shifting Allotment</h3>
                </div>
                @include('housingTheme.partials.alerts')
                
                <form method="GET" action="{{ route('shifting-allotment.cs') }}" id="cs-allotment-form">
                    <div class="row mb-4">
                        <div class="col-md-4">
                ...
                <form method="POST" action="{{ route('shifting-allotment.cs.process') }}" 
                    id="process-cs-allotment-form" 
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <button type="submit" class="btn bg-primary btn-sm px-5 rounded-pill text-white fw-bolder"
                                ...
                                <i class="fa fa-key me-2"></i> Click For Category Shifting Allotment
                            </button>
                        </div>
                    </div>
                </form>
...
function loadCounts() {
    ...
    fetch(`{{ route('shifting-allotment.cs.counts') }}?rhe_id=${rheId}`, {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            const submitBtn = document.querySelector('#process-cs-allotment-form button[type="submit"]');
...
function validateShiftingAllotmentForm() {
    ...
    return confirm('Are you sure you want to process Category Shifting Allotment?');
}
