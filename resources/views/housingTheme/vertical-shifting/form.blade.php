...
@section('title', 'Application for Floor Shifting')
            ...
            <div class="cms-card">
                <div class="cms-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h3><i class="fa fa-arrows-alt-v me-2"></i> Application for Floor Shifting</h3>
                ...
                <form id="vs-application-form" method="POST" action="{{ route('vertical-shifting.store') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="online_vs_id" id="online_vs_id" value="{{ $appData['online_vs_id'] ?? 0 }}">
                    ...
                    {{-- Current Occupency Details (VS Specific) --}}
                    <div class="form-section mt-4">
                        <h5 class="mb-3"><i class="fa fa-home me-2"></i> Current Occupency Details</h5>
                        
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <select class="form-select" id="occupation_estate" name="occupation_estate" required>
                                        <option value="" {{ empty($appData['occupation_estate'] ?? $occupation['estate_id'] ?? old('occupation_estate')) ? 'selected' : '' }}>- Select -</option>
                            ...
                            <div class="col-md-6" id="block_replace">
                                <div class="form-floating">
                                    <select class="form-select" id="occupation_block" name="occupation_block" required>
                                        <option value="" {{ empty($appData['occupation_block'] ?? $occupation['block_id'] ?? old('occupation_block')) ? 'selected' : '' }}>- Select -</option>
                            ...
                            <div class="col-md-6" id="flat_no_replace">
                                <div class="form-floating">
                                    <select class="form-select" id="occupation_flat" name="occupation_flat" required>
                                        <option value="" {{ empty($appData['occupation_flat'] ?? $occupation['flat_id'] ?? old('occupation_flat')) ? 'selected' : '' }}>- Select -</option>
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="text" class="form-control" id="possession_date" name="possession_date" 
                                        value="{{ $appData['possession_date'] ?? old('possession_date', '') }}" 
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="file" class="form-control" id="file_licence" name="file_licence" 
                                        accept=".pdf" required>
                                    <label for="file_licence" class="required">Upload Current Licence</label>
                                    <small class="text-muted">Allowed Extension: pdf | Maximum File Size: 1 MB</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="file" class="form-control" id="vs_scanned_sign" name="vs_scanned_sign" 
                                        accept=".jpg,.jpeg">
                                    <label for="vs_scanned_sign">Upload Scanned Signature</label>
                    ...
                    <div class="row mt-4">
                        <div class="col-12 border-top pt-3">
                            ...
                            <button type="submit" name="apply" class="btn bg-primary btn-sm px-5 rounded-pill text-white fw-bolder" 
                                onclick="document.getElementById('form_action').value='applied'; return validate_vs_application_form();">
...
<script>
    $(document).ready(function() {
        ...
        $('#occupation_estate').on('change', function() {
            ...
            if (estateId) {
                ...
                $('#occupation_block').html('<option value="">- Select -</option>');
                $('#occupation_flat').html('<option value="">- Select -</option>');
            }
        });

        // Block change - load flats
        $('#occupation_block').on('change', function() {
    ...
    function validate_vs_application_form() {
        ...
        if (!$('#file_licence')[0].files.length) {
            alert('Please upload current licence');
            return false;
        }

        // File size validation (1MB = 1048576 bytes)
        const file = $('#file_licence')[0].files[0];
        if (file && file.size > 1048576) {
            alert('The file exceeds 1 MB, the maximum allowed size for uploads.');
            return false;
        }

        // Scanned signature validation (50KB = 51200 bytes)
        const signFile = $('#vs_scanned_sign')[0].files[0];
        if (signFile && signFile.size > 51200) {
            alert('The scanned signature file exceeds 50 KB, the maximum allowed size for uploads.');
            return false;
        }
